...
/*==========================================================================
 * Constants -- DispHSTX uses DMA 14/15 + DMA_IRQ_1, so audio uses DMA_IRQ_0
 *==========================================================================*/
 ...
 * 1536 accommodates Heretic's OPL rate (~1421 frames/tic at 49716 Hz). */
...
void i2s_init(i2s_config_t *config) {
    ...
    /* Configure GPIO pins for the selected PIO */
    uint pio_func = (audio_pio == pio0) ? GPIO_FUNC_PIO0 : GPIO_FUNC_PIO1;
    gpio_set_function(config->data_pin, pio_func);
    gpio_set_function(config->clock_pin_base, pio_func);
    gpio_set_function(config->clock_pin_base + 1, pio_func);
     ...
     * PIO I2S outputs 1 bit per 2 PIO cycles, 32 bits per frame = 64 cycles.
     * divider (8.8 fixed point) = sys_clk * 256 / (sample_rate * 64)
     *                            = sys_clk * 4 / sample_rate */
    uint32_t sys_clk = clock_get_hz(clk_sys);
    uint32_t divider = sys_clk * 4 / config->sample_freq;
    uint32_t actual_rate = sys_clk * 4 / divider;  /* back-calculate for debug */
    printf("I2S clock: sys=%lu Hz, div=%lu.%lu, target=%lu Hz, actual~=%lu Hz\n",
           (unsigned long)sys_clk,
           (unsigned long)(divider >> 8), (unsigned long)(divider & 0xff),
           (unsigned long)config->sample_freq,
